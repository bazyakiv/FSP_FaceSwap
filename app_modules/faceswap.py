...
img_path = "faceswap_image\styopa.png";
...
class FaceSwap:
    ...
    def update(self, detection_result: FaceLandmarkerResult, frame): # pyright: ignore[reportInvalidTypeForm]
        ...
        faceoutline = np.array([face_points[i] for i in FACE_OVAL]) # get all the positions from the oval points list
        ...
        src_pts = np.float32(overlay_faceoutline)
        dst_pts = np.float32(faceoutline)

        
        try:
            [matrix, m_mask] = cv.findHomography(src_pts, dst_pts);
